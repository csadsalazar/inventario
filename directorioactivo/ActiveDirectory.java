...
public class ActiveDirectory {
  ...
  //**************************************************************************
    ...
    public static User getUser(String username, LdapContext context) {
        try {
            ...
            if (domainName != null) {
                ...
                NamingEnumeration<SearchResult> answer = context.search(toDC(domainName), "(& (userPrincipalName=" + principalName + ")(objectClass=user)(memberOf=" + pr.getProperty("miembrode") + ")(&(!( UserAccountControl=514))(!( UserAccountControl=66050))))", controls);
                if (answer.hasMore()) {
    ...
    public static User getUserActive(String username, LdapContext context) {
        try {
            String domainName = null;
            if (username.contains("@")) {
                username = username.substring(0, username.indexOf("@"));
  ...
  //**************************************************************************
    ...
    public static User[] getUsers(LdapContext context) throws NamingException {
        ...
        if (authenticatedUser.contains("@")) {
            ...
            controls.setReturningAttributes(userAttributes);
            PropertiesReader pr = new PropertiesReader("co.gov.invima.utils.cadenaAD");
            //NamingEnumeration answer = context.search( toDC(domainName), "(&(objectCategory=person)(objectClass=user))", controls);
            NamingEnumeration answer = context.search(toDC(domainName), "(&(memberOf=" + pr.getProperty("miembrode") + ")(&(!( UserAccountControl=514))(!( UserAccountControl=66050))))", controls);
            try {
                while (answer.hasMoreElements()) {
                    Attributes attr = ((SearchResult) answer.nextElement()).getAttributes();
                    Attribute user = attr.get("userPrincipalName");
                    if (user != null) {
                ...
                System.out.println("Error users: " + e.toString());
            }
        }
        return users.toArray(new User[users.size()]);
    }

    public static User[] getUsersInactive(LdapContext context) throws NamingException {
