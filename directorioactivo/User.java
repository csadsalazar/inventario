   ...
   */
    public class User {
        ...
        public User(Attributes attr) throws javax.naming.NamingException {
            ...
            member = attr.get("memberOf")!=null?this.miembroDe(attr.get("memberOf").getAll()):null;
        }
 
        private List<String> miembroDe(NamingEnumeration miembro){
            List<String> miembros=null;
            try {
                if(miembro!=null){
                    miembros = new ArrayList<>();
                    while (miembro.hasMore()) {
                        miembros.add((String) miembro.next());
